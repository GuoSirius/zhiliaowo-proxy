 ...
 *
 * decade：按 decadeMode 分两种口径——
 *   `sameRange`：十年每一年（含指定 year）都按 [startMonth, endMonth] 区间本地聚合，忽略 2.4 全年量；
 *   `full`（默认）：倒推 9 年用 2.4 全年优先 → 本地全年 1-12 月补全；指定 year 固定取本地 1~endMonth
 *     聚合（不再用 2.4 全年量），与海报累计文案「截止至 {year} 年 {endMonth} 月」保持一致。
